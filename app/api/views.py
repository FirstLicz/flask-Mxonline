...
def upload():
    ...
    with open(os.path.join(STATIC_DIR, 'ueditor', 'python', 'config.json'), encoding='utf8') as f:
        t = f.read()
        try:
    ...
    if request.method == 'POST':
        if action in ('uploadimage', 'uploadfile', 'uploadvideo'):
            ...
            if fieldName in request.files:
                ...
                uploader = UpLoader(file_obj=file, config=config, upload_path=os.path.join(basedir, 'app', 'static'))
                uploader.up_file()
            ...
            if fieldName in request.form:
                ...
                uploader = UpLoader(file_obj=file, config=config,
                                    upload_path=os.path.join(basedir, 'app', 'static'), _type='base64')
                uploader.up_base64()
                result = uploader.callback_info()
            else:
                result['state'] = '上传接口出错'
